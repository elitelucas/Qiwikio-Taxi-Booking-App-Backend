$(function () {
    ...
    const riderSockets = ['test', 'giftRequest', 'giftAccept', 'giftDecline', 'tripRequest', 'riderCancel', 'autoCancel', 'cancelTripRequest', 'loginCheck', 'rider'];
    ...
    if (userId) {
        if (userType == '1') { // Rider events
            ...
            // Receive - Receive Gift Ride Request from sender
            socket.on('giftRequest-' + userId, async function (responseData) {
                let html = '<span class="eventName">giftRequest-' + userId + ': </span>';
            ...
            // Receive - Receive Gift Ride Request from sender
            socket.on('giftAccept-' + userId, async function (responseData) {
                let html = '<span class="eventName">giftAccept-' + userId + ': </span>';
                html = html + '<span>' + JSON.stringify(responseData) + '</span>';
            ...
            // Receive - Receive Gift Ride Request from sender
            socket.on('giftDecline-' + userId, async function (responseData) {
                let html = '<span class="eventName">giftDecline-' + userId + ': </span>';
                html = html + '<span>' + JSON.stringify(responseData) + '</span>';
