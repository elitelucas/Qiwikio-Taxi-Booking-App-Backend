...
const connection = (app, io) => {
    io.on('connection', function (socket) {
        ...
        /***** 
        * Send gift ride to friend 
        *****/
        socket.on('giftRequest', async function (requestData) {
            let formattedRequest = JSON.parse(requestData);
            let authCheck = await socketVerifyJWT_MW(formattedRequest.data);

            if (formattedRequest) {
                if (authCheck && authCheck.status === 200) {
                    io.emit('giftRequest-' + formattedRequest.data.giftreceiverId, { data: formattedRequest }); // Request to Friend(Accept/Decline)
                } else {
                    io.emit('giftRequest-' + formattedRequest.data.userId, { data: authCheck }); // Request to Rider(Authentical Fail)
                }
            } else {
                io.emit('giftRequest-' + formattedRequest.data.userId, {
                    data: {
                        status: 400,
        ...
        /***** 
        * Send Socket from Gift Receiver to Gift Sender for gift accept
        *****/
        socket.on('giftAccept', async function (requestData) {
            let formattedRequest = JSON.parse(requestData);
            let authCheck = await socketVerifyJWT_MW(formattedRequest.data);

            if (formattedRequest) {
                if (authCheck && authCheck.status === 200) {
                    io.emit('giftAccept-' + formattedRequest.data.giftsenderId, { data: formattedRequest }); // Request to Sender(Accepted)
                } else {
                    io.emit('giftAccept-' + formattedRequest.data.userId, { data: authCheck }); // Request to Receiver(Authentical Fail)
                }
            } else {
                io.emit('giftAccept-' + formattedRequest.data.userId, {
                    data: {
                        status: 400,
                        errorMessage: 'Oops! sorry, something went wrong. Please try again.'
                    }
                }); // Request to Receiver(Required information missing)
            }
        });

        /***** 
        * Send Socket from Gift Receiver to Gift Sender for gift decline
        *****/
        socket.on('giftDecline', async function (requestData) {
            let formattedRequest = JSON.parse(requestData);
            let authCheck = await socketVerifyJWT_MW(formattedRequest.data);

            if (formattedRequest) {
                if (authCheck && authCheck.status === 200) {
                    io.emit('giftDecline-' + formattedRequest.data.giftsenderId, { data: formattedRequest }); // Request to Sender(Accepted)
                } else {
                    io.emit('giftDecline-' + formattedRequest.data.userId, { data: authCheck }); // Request to Receiver(Authentical Fail)
                }
            } else {
                io.emit('giftDecline-' + formattedRequest.data.userId, {
                    data: {
                        status: 400,
                        errorMessage: 'Oops! sorry, something went wrong. Please try again.'
                    }
        ...
        *****/
        socket.on('tripRequest', async function (requestData) {   
            console.log('tripRequest: ' + requestData);         
            let formattedRequest = JSON.parse(requestData);
            let authCheck = await socketVerifyJWT_MW(formattedRequest.data);
            let responseData;
            if (formattedRequest) {
            ...
            if (formattedRequest) {
                if (authCheck && authCheck.status === 200) {
                    ...
                    if (responseData && responseData.status === 200) {
                        ...
                        }); // Request to Driver to inform success
                    } else {
        ...
        socket.on('tripStart', async function (requestData) {
            ...
            if (authCheck && authCheck.status === 200) {
                ...
                if (responseData && responseData.status === 200) {
                    ...
                    io.emit('tripStart-' + formattedRequest.data.userId, {
                        data: {
                            status: 200
                        }
                    }); // Request to Driver to inform success
