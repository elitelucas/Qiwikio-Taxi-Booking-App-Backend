...
import { Booking, ScheduleBooking, ScheduleBookingHistory } from "../../models";
  ...
  async resolve({ request }, { bookingId, scheduleFrom, scheduleTo }) {
    try {
      ...
      if (request && request.user) {
        ...
        const scheduleId = scheduledBookingData.id;

        if (bookingData && bookingData.tripStatus === "scheduled") {
          if (scheduleFrom) {
            formattedScheduleFrom = moment
              .unix(scheduleFrom)
              .set({ s: 0 })
              .format("YYYY-MM-DD HH:mm:ss");
            formattedScheduleTo = moment
              .unix(scheduleTo)
              .set({ s: 0 })
              .format("YYYY-MM-DD HH:mm:ss");
          }

          const { status: eligibleStatus, errorMessage: eligibleErrorMessage } =
          ...
          const { status, errorMessage } = await checkRiderActiveBooking(
            riderId,
            formattedScheduleFrom,
            scheduleId
          );
          if (status !== 200) {
            return {
          ...
          return {
            status: 400,
            errorMessage:
              "Oops! Something went wrong! Please close your application and try again.",
          };
        }
      } else {
        return {
